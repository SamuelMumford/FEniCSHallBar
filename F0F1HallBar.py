...
def solver(g, tol, mesh, omega, alpha, W, Dx, Dy, Dz, Dyz, l):
    ...
    noslip = Constant(0.0)
    Tslip = Expression('x[2] > (l - tol)', degree = 2, l = l, tol = tol)
    Tslip2 = Expression('x[2] < (tol)', degree = 2, l = l, tol = tol)
    ...
    L =  100000*(Tslip-Tslip2)*v1*ds + 100000*(Tslip-Tslip2)*v2*ds

    w = Function(W)
    solve(a == L, w, bcs,solver_parameters={'linear_solver':'mumps', 'preconditioner': 'ilu'})
...
ps = np.linspace(pstart, pstop, runs)
...
for i in range(0, runs):
    ...
    maxi = max(amp)
    mini = min(amp)
    amp = amp - mini/(maxi - mini)
    ampb = ampb - mini/(maxi - mini)
    vals = vals/(maxi - mini)
    s = np.sign(vals)
    if(np.mean(s) < 0):
        vals = vals*-1
