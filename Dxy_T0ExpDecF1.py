...
def solver(g, tol, mesh, omega, alpha, W, Dx, Dy, Dz, Dyz):
...
def doSim(mesh, h, w, l, projx, projy, topS, omega, Dx, Dy, Dz, Dyz, tol, depth, hdepth, projw, projl, ts, den):
    ...
    g = Expression('(omega + 1)*exp(b*(x[2] - top))*(x[0] < (xcenter + wid))*(x[0] >(xcenter - wid))*(x[1] < (ycenter + leng))*(x[1] >(ycenter - leng))', degree = 2, top = zspot, wid = xwid, leng = ywid, xcenter = xcenter, ycenter = ycenter, omega = omega, b = 1/hdepth)
    
    ureal, uimag = solver(g, tol, mesh, omega, alpha1, W, Dx, Dy, Dz, Dyz)
...
plt.xlabel('x (u)')
plt.savefig("TySignal_ExpDecFlot.pdf", bbox_inches='tight')
plt.show()